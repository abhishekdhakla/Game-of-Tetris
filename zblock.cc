...
zBlock::zBlock(char blockType, int level,Board* gameBoard, bool isHeavy):Block(blockType),gameBoard(gameBoard){
    this->level = level;
    orientationType = 0;
    posX = 0;
    posY = 2;
...
zBlock::zBlock(char blockType, int level,BlockCell cellCoords):Block(blockType){
    this->level = level;
...
void zBlock::setCellBlock(int targetOrientation){
